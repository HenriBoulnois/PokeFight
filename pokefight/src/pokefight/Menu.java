...
public class Menu {
	...
	static String regles = "2 - Regles";
	static String progression = "3 - Progression";
	static String classement = "4 - Classement";
	static String pokefighter = "5 - Pokefighter";
	static String MRelges = "            -------------------------\n            | REGLES DE POKEFIGHTER |\n            -------------------------\n" 
			+ "Bienvenu dans PokeFight \n"  
			+ "Choisis ton PokeFighter et affronte tous les boss pour gagner. \n" 
			+ "Chaque Pokefighter et chaque ennemies possedent des types parmis \n "
			+ "les types EAU, PLANTES et FEU.\n"
			+ "Tu es coince dans un donjon et dois t'aider de tes PokeFitghter pour en sortir\n"
			+ "Si jamais tu perd toute ta vie lors d'un combat\n"
			+ "tu dois retourner a  l'entree et tout recommencer ! \n"
			+ "Depuis le menu principal, tu peux lancer une nouvelle partie, consulter tes precedents records, \n"
			+ "ta progression ou renommer tes PokeFighter. \n";
		...
		while(!ok) {
			...
			if(scanedT.matches("[1-5]")) {
				...
				if(scaned == 1) {
					...
					if(nbBossBattu != 5) {System.out.println("Vous avez vaincu ce Boss legendaire ! Le suivant n'a qu'a  bien se tenir !\n\n\n");
					Menu.main(args);
					} 
				} 
			} else {
